...
-- Guardas de producción autorizadas. Identifican la organización por hechos agregados,
-- nunca por una referencia de reserva ni por un código de arrendador.
CREATE TEMP TABLE _ses_manual_guard AS
WITH candidates AS (
  SELECT
    organization_id,
    count(*) FILTER (
      WHERE pickup_at >= TIMESTAMPTZ '2026-08-21 00:00:00+00'
        AND pickup_at < TIMESTAMPTZ '2026-09-28 00:00:00+00'
    ) AS range_drafts,
    count(*) FILTER (
      WHERE pickup_at >= TIMESTAMPTZ '2026-08-21 00:00:00+00'
        AND pickup_at < TIMESTAMPTZ '2026-09-28 00:00:00+00'
...
SELECT
  organization_id,
  TIMESTAMPTZ '2026-08-21 00:00:00+00' AS range_start,
  TIMESTAMPTZ '2026-09-28 00:00:00+00' AS range_end,
  range_drafts, range_ready, range_incomplete, range_accepted, total_accepted
FROM candidates
WHERE range_drafts = 224
  AND range_ready = 19
  AND range_incomplete = 204
  AND range_accepted = 1
  AND total_accepted = 1;

DO $$
DECLARE
  guard record;
BEGIN
  IF (SELECT count(*) FROM _ses_manual_guard) <> 1 THEN
    RAISE EXCEPTION 'SES manual precondition failed: expected one organization with range distribution 224/19/204/1';
  END IF;
  SELECT * INTO guard FROM _ses_manual_guard;
  IF (SELECT count(*) FROM public.ses_settings WHERE organization_id = guard.organization_id) <> 1 THEN
    RAISE EXCEPTION 'SES manual precondition failed: expected one settings row';
  END IF;
  IF (SELECT count(*) FROM public.ses_batches WHERE organization_id = guard.organization_id) <> 1 THEN
    RAISE EXCEPTION 'SES manual precondition failed: expected one batch';
  END IF;
  IF (SELECT count(*) FROM public.ses_batch_items item
      JOIN public.ses_batches batch ON batch.id = item.batch_id
      WHERE batch.organization_id = guard.organization_id) <> 1 THEN
    RAISE EXCEPTION 'SES manual precondition failed: expected one batch item';
  END IF;
  IF (SELECT count(*) FROM public.ses_audit_events WHERE organization_id = guard.organization_id) <> 165 THEN
    RAISE EXCEPTION 'SES manual precondition failed: expected 165 audit events';
  END IF;
  IF NOT EXISTS (
    SELECT 1
    ...
    WHERE batch.organization_id = guard.organization_id
      AND batch.status = 'accepted'
      AND item.result_status = 'accepted'
      AND draft.status = 'accepted'
  ) THEN
    RAISE EXCEPTION 'SES manual precondition failed: accepted batch/item/draft chain is missing';
  END IF;
END;
$$;

-- Capture immutable pre-migration evidence. These temporary tables disappear on COMMIT/ROLLBACK.
CREATE TEMP TABLE _ses_pre_drafts ON COMMIT PRESERVE ROWS AS
SELECT id, organization_id, reference, status, draft_version, content_hash, updated_at
FROM public.ses_contract_drafts;

CREATE TEMP TABLE _ses_pre_batches ON COMMIT PRESERVE ROWS AS
...
CREATE TEMP TABLE _ses_pre_batch_items ON COMMIT PRESERVE ROWS AS
...
CREATE TEMP TABLE _ses_pre_settings ON COMMIT PRESERVE ROWS AS
...
CREATE TEMP TABLE _ses_pre_draft_rows ON COMMIT PRESERVE ROWS AS
SELECT id, to_jsonb(row_value) AS payload FROM public.ses_contract_drafts row_value;
CREATE TEMP TABLE _ses_pre_batch_rows ON COMMIT PRESERVE ROWS AS
SELECT id, to_jsonb(row_value) AS payload FROM public.ses_batches row_value;
CREATE TEMP TABLE _ses_pre_batch_item_rows ON COMMIT PRESERVE ROWS AS
SELECT id, to_jsonb(row_value) AS payload FROM public.ses_batch_items row_value;
CREATE TEMP TABLE _ses_pre_setting_rows ON COMMIT PRESERVE ROWS AS
SELECT id, to_jsonb(row_value) AS payload FROM public.ses_settings row_value;
-- source_hash does not exist in the historical schema. The real protection is the
-- identity plus SHA-256 of every complete historical row and every payload_snapshot.
CREATE TEMP TABLE _ses_manual_pre_draft_hashes AS
SELECT id, encode(digest((to_jsonb(row_value) - ARRAY[
  'is_complete','is_eligible','is_officially_clear','ready_for_xml','eligibility_errors',
  'eligibility_snapshot','official_check_status','last_eligibility_checked_at','document_version'
])::text, 'sha256'), 'hex') AS row_sha256
FROM public.ses_contract_drafts row_value;
CREATE TEMP TABLE _ses_manual_pre_batch_hashes AS
SELECT id, encode(digest((to_jsonb(row_value) - ARRAY[
  'official_lot_code','document_version','xsd_version','xsd_hash','xsd_validated_at'
])::text, 'sha256'), 'hex') AS row_sha256
FROM public.ses_batches row_value;
CREATE TEMP TABLE _ses_manual_pre_item_hashes AS
SELECT id,
       encode(digest((to_jsonb(row_value) - ARRAY[
         'official_communication_code','snapshot_version','snapshot_hash'
       ])::text, 'sha256'), 'hex') AS row_sha256,
       encode(digest(payload_snapshot::text, 'sha256'), 'hex') AS payload_sha256
FROM public.ses_batch_items row_value;
CREATE TEMP TABLE _ses_manual_pre_settings_hashes AS
SELECT id, encode(digest((to_jsonb(row_value) - ARRAY[
...
FROM public.ses_settings row_value;
CREATE TEMP TABLE _ses_manual_pre_audit_hashes AS
SELECT id, encode(digest(to_jsonb(row_value)::text, 'sha256'), 'hex') AS row_sha256
FROM public.ses_audit_events row_value;
CREATE TEMP TABLE _ses_manual_pre_person_hashes AS
SELECT id, encode(digest(to_jsonb(row_value)::text, 'sha256'), 'hex') AS row_sha256
FROM public.ses_person_profiles row_value;
CREATE TEMP TABLE _ses_manual_pre_location_hashes AS
SELECT id, encode(digest(to_jsonb(row_value)::text, 'sha256'), 'hex') AS row_sha256
FROM public.ses_locations row_value;
...
-- Manual production postconditions. Any failure raises before COMMIT and rolls back all DDL/DML.
DO $$
DECLARE
  guard record;
BEGIN
  SELECT * INTO guard FROM _ses_manual_guard;
  IF guard.organization_id IS NULL THEN
    RAISE EXCEPTION 'SES manual postcondition failed: production guard is missing';
  END IF;
  IF (SELECT count(*) FROM public.ses_contract_drafts
      WHERE organization_id = guard.organization_id
        AND pickup_at >= guard.range_start AND pickup_at < guard.range_end) <> 224
     OR (SELECT count(*) FROM public.ses_contract_drafts
         WHERE organization_id = guard.organization_id
           AND pickup_at >= guard.range_start AND pickup_at < guard.range_end AND status = 'ready') <> 19
     OR (SELECT count(*) FROM public.ses_contract_drafts
         WHERE organization_id = guard.organization_id
           AND pickup_at >= guard.range_start AND pickup_at < guard.range_end AND status = 'incomplete') <> 204
     OR (SELECT count(*) FROM public.ses_contract_drafts
         WHERE organization_id = guard.organization_id
           AND pickup_at >= guard.range_start AND pickup_at < guard.range_end AND status = 'accepted') <> 1 THEN
    RAISE EXCEPTION 'SES manual postcondition failed: range distribution is not 224/19/204/1';
  END IF;
  IF (SELECT count(*) FROM public.ses_contract_drafts
      WHERE organization_id = guard.organization_id AND status = 'accepted') <> 1
     OR (SELECT count(*) FROM public.ses_settings WHERE organization_id = guard.organization_id) <> 1
     OR (SELECT count(*) FROM public.ses_batches WHERE organization_id = guard.organization_id) <> 1
     OR (SELECT count(*) FROM public.ses_batch_items item
         JOIN public.ses_batches batch ON batch.id = item.batch_id
         WHERE batch.organization_id = guard.organization_id) <> 1
     OR (SELECT count(*) FROM public.ses_audit_events WHERE organization_id = guard.organization_id) <> 165 THEN
    RAISE EXCEPTION 'SES manual postcondition failed: accepted/settings/batch/item/audit counts changed';
  END IF;
  IF EXISTS (
    SELECT 1 FROM _ses_manual_pre_draft_hashes before_row
    LEFT JOIN public.ses_contract_drafts after_row USING (id)
    WHERE after_row.id IS NULL
       OR before_row.row_sha256 IS DISTINCT FROM encode(digest(
         (to_jsonb(after_row) - ARRAY[
           'is_complete','is_eligible','is_officially_clear','ready_for_xml','eligibility_errors',
           'eligibility_snapshot','official_check_status','last_eligibility_checked_at','document_version'
         ])::text, 'sha256'), 'hex')
  ) THEN
    RAISE EXCEPTION 'SES manual postcondition failed: historical draft identity or real legacy columns changed';
  END IF;
  IF EXISTS (
    SELECT 1 FROM _ses_manual_pre_batch_hashes before_row
    LEFT JOIN public.ses_batches after_row USING (id)
    WHERE after_row.id IS NULL
       OR before_row.row_sha256 IS DISTINCT FROM encode(digest(
         (to_jsonb(after_row) - ARRAY[
           'official_lot_code','document_version','xsd_version','xsd_hash','xsd_validated_at'
    ...
    RAISE EXCEPTION 'SES manual postcondition failed: historical batch identity or real legacy columns changed';
  END IF;
  IF EXISTS (
    SELECT 1 FROM _ses_manual_pre_item_hashes before_row
    LEFT JOIN public.ses_batch_items after_row USING (id)
    WHERE after_row.id IS NULL
       OR before_row.row_sha256 IS DISTINCT FROM encode(digest(
         (to_jsonb(after_row) - ARRAY['official_communication_code','snapshot_version','snapshot_hash'])::text,
         'sha256'), 'hex')
       OR before_row.payload_sha256 IS DISTINCT FROM encode(digest(after_row.payload_snapshot::text, 'sha256'), 'hex')
  ) THEN
    RAISE EXCEPTION 'SES manual postcondition failed: batch item identity, legacy columns or payload_snapshot changed';
  END IF;
  IF EXISTS (
    SELECT 1 FROM _ses_manual_pre_settings_hashes before_row
    LEFT JOIN public.ses_settings after_row USING (id)
    WHERE after_row.id IS NULL
       OR before_row.row_sha256 IS DISTINCT FROM encode(digest(
         (to_jsonb(after_row) - ARRAY[
           'official_xsd_storage_key','official_xsd_url','official_xsd_hash','official_xsd_version',
    ...
    RAISE EXCEPTION 'SES manual postcondition failed: settings identity or real legacy columns changed';
  END IF;
  IF EXISTS (
    SELECT 1 FROM _ses_manual_pre_audit_hashes before_row
    LEFT JOIN public.ses_audit_events after_row USING (id)
    WHERE after_row.id IS NULL OR before_row.row_sha256 IS DISTINCT FROM encode(digest(to_jsonb(after_row)::text, 'sha256'), 'hex')
  ) OR EXISTS (
    SELECT 1 FROM _ses_manual_pre_person_hashes before_row
    LEFT JOIN public.ses_person_profiles after_row USING (id)
    WHERE after_row.id IS NULL OR before_row.row_sha256 IS DISTINCT FROM encode(digest(to_jsonb(after_row)::text, 'sha256'), 'hex')
  ) OR EXISTS (
    SELECT 1 FROM _ses_manual_pre_location_hashes before_row
    LEFT JOIN public.ses_locations after_row USING (id)
    WHERE after_row.id IS NULL OR before_row.row_sha256 IS DISTINCT FROM encode(digest(to_jsonb(after_row)::text, 'sha256'), 'hex')
  ) THEN
    RAISE EXCEPTION 'SES manual postcondition failed: audit, person or location history changed';
  END IF;
  IF EXISTS (
    SELECT 1 FROM public.ses_contract_drafts
    WHERE organization_id = guard.organization_id AND ready_for_xml = true
  ) THEN
    RAISE EXCEPTION 'SES manual postcondition failed: a legacy draft is ready_for_xml without general revalidation';
  END IF;
  IF EXISTS (
    SELECT 1 FROM public.ses_settings
    WHERE organization_id = guard.organization_id
      AND (official_xsd_storage_key IS NOT NULL OR official_xsd_url IS NOT NULL
        OR official_xsd_hash IS NOT NULL OR official_xsd_version IS NOT NULL
        OR official_xsd_uploaded_at IS NOT NULL OR official_xsd_uploaded_by IS NOT NULL
        OR official_inventory_confirmed_at IS NOT NULL OR official_inventory_source_date IS NOT NULL
        OR official_inventory_confirmed_by IS NOT NULL)
  ) THEN
    RAISE EXCEPTION 'SES manual postcondition failed: XSD or official inventory was enabled without authorization';
  END IF;
  IF to_regclass('public.ses_official_communications') IS NULL
     OR to_regclass('public.ses_historical_snapshots') IS NULL
     OR to_regclass('public.ses_field_audit_events') IS NULL THEN
    RAISE EXCEPTION 'SES manual postcondition failed: a required compatibility table is missing';
  END IF;
  IF EXISTS (
    SELECT 1 FROM pg_class table_info
    WHERE table_info.oid IN (
      'public.ses_official_communications'::regclass,
      'public.ses_historical_snapshots'::regclass,
      'public.ses_field_audit_events'::regclass
    ) AND NOT table_info.relrowsecurity
  ) THEN
    RAISE EXCEPTION 'SES manual postcondition failed: RLS is not enabled on every compatibility table';
  END IF;
  IF has_table_privilege('anon', 'public.ses_official_communications', 'SELECT')
     OR has_table_privilege('authenticated', 'public.ses_official_communications', 'SELECT')
     OR has_table_privilege('anon', 'public.ses_historical_snapshots', 'SELECT')
     OR has_table_privilege('authenticated', 'public.ses_historical_snapshots', 'SELECT')
     OR has_table_privilege('anon', 'public.ses_field_audit_events', 'SELECT')
     OR has_table_privilege('authenticated', 'public.ses_field_audit_events', 'SELECT')
     OR NOT has_table_privilege('service_role', 'public.ses_official_communications', 'SELECT')
     OR NOT has_table_privilege('service_role', 'public.ses_historical_snapshots', 'SELECT')
     OR NOT has_table_privilege('service_role', 'public.ses_field_audit_events', 'SELECT') THEN
    RAISE EXCEPTION 'SES manual postcondition failed: grants are not service-role only';
  END IF;
  IF EXISTS (
    SELECT 1
    FROM (VALUES
      ('reservations','rently_delivery_branch_office_id'),
      ('reservations','rently_delivery_actual_at'),
      ('reservations','rently_detail_booking_id'),
      ('reservations','rently_detail_vehicle_plate'),
      ('ses_settings','official_xsd_storage_key'),
      ('ses_settings','official_inventory_confirmed_at'),
      ('ses_contract_drafts','is_complete'),
      ('ses_contract_drafts','is_eligible'),
      ('ses_contract_drafts','is_officially_clear'),
      ('ses_contract_drafts','ready_for_xml'),
      ('ses_contract_drafts','official_check_status'),
      ('ses_batches','official_lot_code'),
      ('ses_batches','xsd_validated_at'),
      ('ses_batch_items','official_communication_code'),
      ('ses_batch_items','snapshot_hash')
    ...
    RAISE EXCEPTION 'SES manual postcondition failed: a required compatibility column is missing';
  END IF;
END;
$$;
