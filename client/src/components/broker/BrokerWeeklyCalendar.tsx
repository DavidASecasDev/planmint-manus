/**
 * BrokerWeeklyCalendar — Weekly calendar view for the broker portal
 * Shows hour-by-hour detail with drivers, routes, and status indicators
 * Adapted from TransfersWeeklyCalendar with broker-specific routing
 */
...
interface BrokerWeeklyCalendarProps {
...
export function BrokerWeeklyCalendar({ requests }: BrokerWeeklyCalendarProps) {
  ...
  const eventsByDayHour = useMemo(() => {
    ...
    return map;
  }, [requests, weekDays, statusFilter]);
  ...
  const getStatusBg = (status: TransferRequestStatus) => {
    switch (status) {
      case 'pendiente': return 'bg-amber-50 border-amber-200';
      case 'aceptado':
      case 'conductor_asignado': return 'bg-blue-50 border-blue-200';
      case 'en_curso': return 'bg-orange-50 border-orange-200';
      case 'completado': return 'bg-emerald-50 border-emerald-200';
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
        <div className="flex items-center gap-2">
          ...
          <h2
            className="text-base min-w-[220px] text-center text-foreground"
            ...
            <div className="grid grid-cols-[60px_repeat(7,1fr)] border-b bg-muted/30 sticky top-0 z-10">
              ...
              {weekDays.map((day, idx) => {
                ...
                return (
                  ...
                  >
                    <div className={`text-xs font-medium capitalize ${isCurrentDay ? 'text-primary' : 'text-muted-foreground'}`}>
                      {format(day, 'EEE', { locale: es })}
                    </div>
                    <div className={`text-sm font-bold ${isCurrentDay ? 'text-primary' : ''}`}>
                      {format(day, 'd')}
                    </div>
                    {count > 0 && (
                                ...
                                {evt.vehicleType && (
                                  <span className="flex items-center gap-0.5 text-[9px] text-muted-foreground ml-auto">
                                    ...
                                    <span className="truncate max-w-[50px]">
                                      {evt.vehicleType.includes('vclass') ? 'V-Class' : evt.vehicleType.includes('vito') ? 'Vito' : evt.vehicleType}
                                    </span>
                                  </span>
                                )}
                              </div>
      ...
      <div className="flex flex-wrap items-center gap-3 text-xs text-muted-foreground" style={{ fontFamily: 'Barlow, sans-serif' }}>
        <span className="flex items-center gap-1.5">
          <div className="w-3 h-3 rounded bg-amber-50 border border-amber-300" /> Pendiente
        </span>
        <span className="flex items-center gap-1.5">
