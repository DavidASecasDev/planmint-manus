      ...
      <div className="grid gap-3 grid-cols-2 lg:grid-cols-5">
        ...
        >
          <CardContent className="p-4">
            <div className="flex items-center justify-between mb-2">
              ...
              {needsPrep > 0 && (
                <Badge variant="outline" className="text-orange-500 border-orange-500/30 text-[10px] px-1.5 py-0">
                  {needsPrep} preparar
                </Badge>
              )}
            </div>
            <div className="text-2xl font-bold text-foreground">{stats.vehiclesByStatus.limpio}</div>
              ...
              <div className="flex gap-1">
                ...
                {stats.todayCheckOuts > 0 && (
                  <Badge variant="outline" className="text-amber-600 border-amber-500/30 text-[10px] px-1.5 py-0">
                    <ArrowUpFromLine className="h-2.5 w-2.5 mr-0.5" />
        ...
        >
          <CardContent className="p-4">
            ...
            <div className="text-2xl font-bold text-foreground">{stats.pendingTasksTotal}</div>
          ...
          <CardHeader className="pb-2">
            <div className="flex items-center justify-between">
              <CardTitle className="text-sm font-medium text-muted-foreground flex items-center gap-2">
                ...
                Operaciones de hoy · {stats.todayCheckIns + stats.todayCheckOuts} movimientos
              </CardTitle>
              <Button variant="ghost" size="sm" className="gap-1 text-xs h-7" onClick={() => navigate('/reservations')}>
          ...
          <CardContent className="pb-3">
            <div className="divide-y divide-border/50">
              {stats.todayReservations.slice(0, 8).map((r) => (
                <div
                  key={`${r.id}-${r.type}`}
                  className="flex items-center gap-3 py-2.5 text-sm cursor-pointer hover:bg-muted/30 -mx-2 px-2 rounded transition-colors"
                  onClick={() => navigate('/reservations')}
                >
                  {/* Type indicator */}
                  <div className={`flex items-center justify-center h-7 w-7 rounded-full flex-shrink-0 ${
                    r.type === 'checkin'
                  ...
                  {/* Location */}
                  <span className="text-xs text-muted-foreground truncate max-w-[100px] flex-shrink-0 hidden sm:block">
          ...
          <CardHeader className="pb-2">
            <div className="flex items-center justify-between">
              <CardTitle className="text-sm font-medium text-muted-foreground flex items-center gap-2">
                ...
                Vehículos por preparar · {needsPrep}
              </CardTitle>
              <Button variant="ghost" size="sm" className="gap-1 text-xs h-7" onClick={() => navigate('/vehicles')}>
                Estado Coches <ChevronRight className="h-3 w-3" />
              </Button>
            </div>
          </CardHeader>
          <CardContent className="pb-3">
            <div className="flex flex-wrap gap-2">
              {stats.vehiclesNeedingPrep.map((v) => (
                <div
                  ...
                  onClick={() => navigate('/vehicles')}
                >
                  <div className={`h-2 w-2 rounded-full ${STATUS_COLORS[v.status]}`} />
          ...
          <CardContent className="p-4">
            ...
            <div className="flex flex-wrap gap-3 text-sm">
              {stats.contractsExpiringSoon > 0 && (
                <div
                  className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-background border border-border/50 cursor-pointer hover:bg-muted/30 transition-colors"
