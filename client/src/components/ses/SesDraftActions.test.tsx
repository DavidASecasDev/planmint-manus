...
function draft(status: SesContractDraft['status'], complete = false): SesContractDraft {
  return { id: 'draft-synthetic', reference: 'SYNTHETIC-1', status, is_complete: complete, is_eligible: false,
    ...
    validation_errors: [], manual_fields: [], draft_version: 1, updated_at: '2026-08-28T00:00:00Z' } as unknown as SesContractDraft;
}

describe('acciones operativas por contrato SES', () => {
  it.each([['incomplete', true], ['pending_sync', false]] as const)('muestra Comprobar SES antes de Completar para %s', (status, complete) => {
