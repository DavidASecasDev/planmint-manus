...
import { Loader2, Building2, ArrowRight, Sparkles, Users, Briefcase, Truck, Mail, CheckCircle2 } from 'lucide-react';
import { z } from 'zod';
...
export default function CreateOrganization() {
  ...
  // Standard create organization form
  return (
    <div className="flex min-h-screen items-center justify-center bg-gradient-to-br from-background via-background to-muted p-4">
      <div className="w-full max-w-lg animate-in">
        <Card className="border-border/50 shadow-xl">
          <CardHeader className="space-y-1 text-center pb-6">
            ...
            <CardDescription className="text-muted-foreground">
              Crea tu organización para comenzar a gestionar tu equipo
            </CardDescription>
          </CardHeader>
          <form onSubmit={handleSubmit}>
            <CardContent className="space-y-6">
              ...
              <div className="space-y-3">
                <Label className="text-sm font-medium">¿Qué tipo de organización eres? (opcional)</Label>
                <RadioGroup
                ...
                >
                  {Object.values(VERTICAL_PRESETS).map((preset) => (
                    <div key={preset.key}>
                      <RadioGroupItem value={preset.key} id={preset.key} className="peer sr-only" />
                      <Label
                        htmlFor={preset.key}
            ...
            <CardFooter className="flex-col gap-3 pt-2">
              ...
              {pendingInvitations.length > 0 && (
                ...
                >
                  ← Volver a invitaciones pendientes
                </Button>
              )}
            </CardFooter>
          </form>
