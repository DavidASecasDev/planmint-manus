-- SES.HOSPEDAJES hardening: eligibility gates, official inventory,
-- immutable historical snapshots, structured official codes and field audit.
...
ALTER TABLE public.ses_contract_drafts
  DROP CONSTRAINT IF EXISTS ses_drafts_official_check_status;
ALTER TABLE public.ses_contract_drafts
...
ALTER TABLE public.ses_contract_drafts
  DROP CONSTRAINT IF EXISTS ses_drafts_status;
ALTER TABLE public.ses_contract_drafts
  ADD CONSTRAINT ses_drafts_status CHECK (status IN (
    'pending_sync','incomplete','ready','batched','uploaded_pending_result',
...
CREATE UNIQUE INDEX IF NOT EXISTS uq_ses_batches_official_lot_code
  ON public.ses_batches(official_lot_code)
...
DROP TRIGGER IF EXISTS trg_ses_historical_snapshots_immutable ON public.ses_historical_snapshots;
...
DROP TRIGGER IF EXISTS trg_ses_official_communications_updated_at ON public.ses_official_communications;
...
ALTER TABLE public.ses_audit_events
  DROP CONSTRAINT IF EXISTS ses_audit_entity_type;
ALTER TABLE public.ses_audit_events
  ADD CONSTRAINT ses_audit_entity_type CHECK (
    entity_type IN ('person','location','draft','batch','batch_item','settings','official_communication','xsd')
  );
...
UPDATE public.ses_batch_items item
SET official_communication_code = '4b86dae9-a184-11f1-ab80-00505695dcc7'
FROM public.ses_contract_drafts draft
WHERE item.draft_id = draft.id
  AND draft.reference = '4942'
  AND item.result_status = 'accepted'
  AND item.official_communication_code IS NULL;
...
SELECT
  ...
  '4b86dae9-a184-11f1-ab80-00505695dcc7',
